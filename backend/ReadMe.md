...
Backend para la plataforma educativa de windsurf con foil. Tecnologías: Node.js, Express, PostgreSQL, MongoDB, Elasticsearch.
...
5. Ejecutar seed (datos de prueba): `npm run seed:postgres` (debes crear el script) y `npm run seed:mongo`
6. Iniciar servidor: `npm run dev` (desarrollo) o `npm start` (producción)

## Estructura

- `src/config/`: configuraciones de bases de datos
- `src/controllers/`: lógica de endpoints
- `src/models/`: modelos de datos (PostgreSQL con consultas parametrizadas, MongoDB con esquemas)
